...
class StandardPredictor(_AbstractPredictor):
    ...
    def _save_results(
        self, prediction_maps, normalization_masks, output_heads, output_file, dataset
    ):
        ...
        with torch.no_grad():
            for img, path in test_loader:
                ...
                for single_pred, single_path in zip(pred, path):
                    ...
                    if (
                        hasattr(test_loader.dataset, "mirror_padding")
                        and test_loader.dataset.mirror_padding is not None
                    ):
                        z_s, y_s, x_s = [
                            self._slice_from_pad(p)
                            for p in test_loader.dataset.mirror_padding
                        ]
                    ...
                    with h5py.File(out_file, "w") as f:
                        ...
                        if self.save_segmentation:
                            f.create_dataset(
                                "segmentation",
                                data=self._pmaps_to_seg(single_pred),
                                compression="gzip",
                            )
